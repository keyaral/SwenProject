...
//	if (safe == false) throw new Exception("Error in Input: Can't detect type.");
	...
	private boolean checkDetails(String details,String type) throws Exception {
		...
		if(type.equals("1a")){
			...
			if(destination == null || origin == null || day == null || companyName == null) throw new Exception("Null Values Entered in route adding - strings");}
			catch(Exception e){throw new Exception("Null Values Entered in route adding - ints");}
		}
		if(type.equals("1b")){
			try{
				int ID = Integer.parseInt(values[0]);
				int costWeight = Integer.parseInt(values[3]);
				int costVolume = Integer.parseInt(values[4]);
			}catch(Exception e){throw new Exception("Error in entering an ID or cost values");}
		}
		if(type.equals("1c")){
			try{
				int ID = Integer.parseInt(values[0]);}catch(Exception e){throw new Exception("Error in entering an ID to delete");}
		}
		if(type.equals("2a")){
			try{
			int ID = Integer.parseInt(values[0]);
			...
			if(destination == null || origin == null)throw new Exception("Null Values Entered in cost adding - strings");
			}catch(Exception e){ throw new Exception("Null values entered in cost adding - ints");}
	}
		if(type.equals("2b")){
			try{
			int ID = Integer.parseInt(values[0]);
			...
			catch(Exception e){throw new Exception("Error in entering an ID or values");}
		}
		if(type.equals("2c")){
			try{
				int ID = Integer.parseInt(values[0]);}catch(Exception e){throw new Exception("Error in entering an ID to delete");}
		...
		if(type.equals("3a")){
			...
			Date date = new Date();
			date.parse(values[6]);
			if(destination == null || origin == null || date == null) throw new Exception("Null values in mail creation for destination/origin or date");
